...
function alert(content: string, config: ElMessageBoxOptions = {}) {
  ElMessageBox.alert(content, '系统提示', { showClose: false, ...config })
}

/** 错误提示 */
function alertError(content: string, config: ElMessageBoxOptions = {}) {
  ElMessageBox.alert(content, '系统提示', { type: 'error', showClose: false, ...config })
}

/** 成功提示 */
function alertSuccess(content: string, config: ElMessageBoxOptions = {}) {
  ElMessageBox.alert(content, '系统提示', { type: 'success', showClose: false, ...config })
}

/** 警告提示 */
function alertWarning(content: string, config: ElMessageBoxOptions = {}) {
  ElMessageBox.alert(content, '系统提示', { type: 'warning', showClose: false, ...config })
}
